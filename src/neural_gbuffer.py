...
class NeuralGBufferExperiment(pl.LightningModule):
    ...
    def __init__(self, model: BaseRenderer, params: dict) -> None:
        super().__init__()
        self.model = model
        self.params = params
    ...
    def sample_images(self, plot: dict, val_indices: Tensor):
        ...
        batch = [self.sample_dataloader.dataset[int(i)] for i in val_indices]
...
def neural_gbuffer(config: dict, run_args: dict) -> pl.LightningModule:
    ...
    image_size = exp_params['data']['training']['rasterization_settings'][
        'image_size']
