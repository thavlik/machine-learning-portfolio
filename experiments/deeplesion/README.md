...
***Figure 1: a visualization of model performance on the validation data. This is how well the model could reasonably be expected to perform on novel data. The ground truth (physician's judgment) is in yellow, and the model's prediction is in green.***
